...
Face_Images = os.path.join(os.getcwd(), "images1")
print(Face_Images)

# go to the face image directory
for root, dirs, files in os.walk(Face_Images):
    for file in files:  # check every directory in it
        if file.endswith("jpeg") or file.endswith("jpg") or file.endswith("png"):
            # for image files ending with jpeg,jpg or png
            path = os.path.join(root, file)
            person_name = os.path.basename(root)
            print(path, person_name)

        if pev_person_name != person_name:
            # Check if the name of person has changed
            Face_ID = Face_ID+1  # If yes increment the ID count
            pev_person_name = person_name

        # convert the image to greysclae using Pillow
        Gery_Image = Image.open(path).convert("L")
        # .resize( (640,640) , Image.ANTIALIAS) #Crop the Grey Image to 550*550 (Make sure your face is in the center in all image)
        Crop_Image = Gery_Image
        Final_Image = np.array(Crop_Image, "uint8")
        # print(Numpy_Image)
        # Detect The face in all sample image
        faces = face_cascade.detectMultiScale(
            Final_Image, scaleFactor=1.5, minNeighbors=5)
        print(Face_ID, faces)
